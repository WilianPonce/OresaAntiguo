<?php  
    ...
    if($_GET["buscar"]!=""){
        $buscar = $_GET["buscar"];     
        $like= "like '%$buscar%'";
        $where = "WHERE ordpedido $like OR CONCAT(razonSocialNombres,' ',razonComercialApellidos) $like OR empleado $like OR fechaInicio $like";
        ...
        $sinb = "SELECT *, (SELECT count(*) FROM (SELECT count(*) FROM vistaordtrabajo GROUP BY ordpedido) d1) as pag FROM vistaordtrabajo GROUP BY ordpedido ORDER BY fechaInicio DESC limit $hj,10";
