var app = new Vue({
    ...
    data: {
        ...
        lisstockl:0,
        //kits
        listar_producto_buscar:[],
        producto_buscar:"",
        producto_buscar_lista:[],
    }, 
    methods: {
        //kits
        listar_productos_para_kits(){
            if(this.producto_buscar.length>=1){
                $("#ver_productos_id1").show();
                axios({
                    url: "modelo/kits/listar_productos_para_kits.php",
                    method: "get",
                    params: {
                        'buscar':this.producto_buscar,
                    }
                }).then(({data}) => {
                    this.producto_buscar_lista = data;
                }).catch((error) => {
                    console.log(error);
                    alertify.error('Error inseperado intente mas tarde o reportelo al administrador'); 
                });
            }else{
                $("#ver_productos_id1").hide();
                this.producto_buscar_lista = [];
            }
        },
        listar() {
            let me = this;
            axios({
                url: "modelo/kits/listar.php",
                method: "get",
                params: {
                    'buscar':this.buscar,
                    'hj':this.hj,
        ...
        registrar() {
            ...
            axios.post('modelo/kits/guardar.php', serialize({
                'codigo':this.codigo, 
                'Safi':this.Safi, 
                ...
                'listar_producto_buscar': this.listar_producto_buscar
            })).then(({data}) => {
                if($.trim(data)=="bien"){
                    this.hj=1;
                    this.cerrarModal();
        ...
        actualizar() {
            ...
            axios.post("modelo/kits/editar.php", serialize({
        ...
        agregar_producto_buscar_lista(tr){
            $("#ver_productos_id1").hide();
            this.producto_buscar = "";
            this.listar_producto_buscar.push(tr);
        },
        eliminar_producto_buscar(index){
            this.listar_producto_buscar.splice(index,1);
        },
        //otros   
        listarmarcas() {
            let me = this;
                ...
                case "stock":
                {
                    switch (accion) {
                        case 'registrar':
                        {
                            ...
                            //necesarios
                            this.modal = 1;
                            this.tituloModal = "Registrar Kit";
                            this.tipoAccion = 1;
                            //complementos
                            this.codigo =  '';
                            this.Safi =  '';
                        ...
                        case 'actualizar':
                        {
                            ...
                            axios.post('modelo/kits/recuperar_productos_para_kits.php', serialize({
                                id:this.idProducto
                            })).then(({data}) => {
                                this.listar_producto_buscar = data;
                            });
                            break;
                        }
                        case 'imprimir':
        ...
        validarg() {
            ...
            if(this.listar_producto_buscar.length>=1){
                for(var i=0; i<this.listar_producto_buscar.length; i++){
                    this.listar_producto_buscar[i].error_valor_cantidad = [];
                    if(this.listar_producto_buscar[i].valor_cantidad>0){}else{ 
                        this.listar_producto_buscar[i].error_valor_cantidad.push("Obligatorio");
                        this.error=1;
                    }
                }
            }
...
$(document).on("click",function(e) {           
    var container = $(".contenedor_lista_general");
        if (!container.is(e.target) && container.has(e.target).length === 0) { 
            $(".contenedor_lista_general").hide();              
        }
});